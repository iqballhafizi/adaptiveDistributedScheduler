...
#
# MODIFIED, Python3, VIRTENIO Gmbh, 2018
...
try:
    ...
    print("For prettier printing please install progressbar2 with:")
    print("python -m pip install progressbar2")
    print("")
    usepbar = 0
...
chip_ids = {
    ...
    0x411: "STM32F2xx",
    0x413: "STM32F4xx",
    0x462: "STM32L45xxx and STM32L46xxx devices"
}
...
class CommandInterface:
    ...
    def _wait_for_ask(self, info = ""):
        # Bootloader rev 22 returns a 0x80 before ACK (0x79).
        # Wait until an ACK or NACK is received
        for i in range(READ_RETRIES+1):
            # wait for ask
            try:
                ask = self.sp.read()
                ask = ask[0]
        ...
        # Unknown responce
        raise CmdException("Unknown response: ", hex(ask))

    def reset(self):
        self.sp.setDTR(0)
        time.sleep(0.1)
        # Flush buffer contents; required if an app outputs contents on the serial
    ...
    def cmdGeneric(self, cmd):
        ...
        return self._wait_for_ask("CMD Generic, " + hex(cmd))
    ...
    def cmdGet(self):
        if self.cmdGeneric(0x00):
            ...
            cmdlist = list(map(hex, list(dat)))
            if '0x44' in cmdlist:
                self.extended_erase = 1
            mdebug(10, "    Available commands: "+str(cmdlist))
    ...
    def cmdGo(self, addr):
        if self.cmdGeneric(0x21):
            ...
            while (True):
                sys.stdout.flush()
                print(self.sp.read().decode('ascii', "ignore"), end='')
        else:
